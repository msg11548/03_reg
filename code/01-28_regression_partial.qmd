...
```{r reg exp boxplot}
ggplot(data = reg_dfw),aes(x = sr_ksha, 
           y = yield_mgha)) + 
  geom_boxplot()
```
...
*Has to be at least 2 points difference to determine which one is better on AIC, doesn't change means adding that model parameter doesn't make a difference*
*lower is better, so the best model in the linear-plateau model*
*AIC model fit given your data, and is unbounded value and can be negative*
